function SocketHandler(socket, io, rm) {
  ...
  socket.on('cloneCharacter', this.onCloneChar.bind(this));
...
SocketHandler.prototype.onCloneChar = function (data) {
  if (!this.gm) {
    return;
  }

  var oldChar = this.room.charGet(data.key);
  var newChar = this.room.charCreate();

  for (var k in oldChar) {
    if (k != 'key') {
      newChar[k] = oldChar[k];
    }
  }

  this.pushCharacterCreate(newChar);
}
